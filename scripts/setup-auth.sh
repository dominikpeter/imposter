...
# Skip a provider by leaving its ID empty. For Google you can give the path to the downloaded client JSON instead.
# Callback URLs to register with each provider:
#   https://whoislying.ch/api/auth/callback/<google|github|microsoft>
...
printf 'https://whoislying.ch' | vercel env add BETTER_AUTH_URL production >/dev/null && echo "  BETTER_AUTH_URL set"
echo "Done. Put the keys live with: just redeploy"
