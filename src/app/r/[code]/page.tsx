...
export default function Room() {
  ...
  if (!hydrated) return <main className="flex-1" />; // no start-screen splash here: the room layout differs, it would jump
        ...
        <div key="lobby" className="enter flex flex-1 flex-col gap-4">
          <section className={`${card} flex flex-col items-center gap-3 text-center`}>
            ...
            <div className="flex flex-wrap justify-center gap-2">
              ...
              {/* straight into a WhatsApp chat with a ready-made invite (wa.me works on phones and desktop) */}
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`${f("invite", code)} ${url}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className={`${ghost} flex items-center gap-2 border border-line`}
              >
...
// WhatsApp glyph (Simple Icons, CC0); lucide has no brand icons
function WhatsAppIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5 shrink-0 fill-whatsapp" aria-hidden>
